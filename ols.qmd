...
## Limiting Theory for i.i.d Random Variables

::: {#thm-lln}
## Law of Large Numbers

Let $\{x_i\}$ be $iid$ random variables with $\mathbb{E}(x_i)=\mu$ and $\text{Var}(x_i)=\sigma^2<\infty$. Define $\bar{x}_n = \frac{1}{n}\sum_{i=1}^n x_i$. Then $\bar{x}_n \overset{p}{\to} \mu$ as $n \to \infty$.
:::

::: proof
We will give an non-rigorous proof, but nonetheless shows the tenets. It is easy to see $\mathbb{E}(\bar{x}_n) = \mu$. Consider the variance,
...
Let $\{x_i\}$ be $iid$ random variables with $\mathbb{E}(x_i)=\mu$ and $\text{Var}(x_i)=\sigma^2<\infty$. Define $\bar{x}_n = \frac{1}{n}\sum_{i=1}^n x_i$. Then
...
::: proof
Without loss of generality, assume $x_i$ is demeaned and standardized to have standard deviation 1. It remains to show $\sqrt{n} \bar x_n \to N(0,1)$. Define the moment generating function (MGF) for $\sqrt{n}\bar x_n$:
