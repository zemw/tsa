...
{
  plot(brownian(1), type = "l", ylim = c(-.1,.1))
  for(i in 1:20) 
    lines(brownian(1), col = rgb(0,0,0,runif(1,0,.3)))
}
```
...
Let $y_t$ be a unit root process,
...
Consider the sum of squared $y_t$:
...
So far we have assumed $\epsilon_t$ are not autocorrelated. This
assumption can be relieved. Let

$$
y_t = y_{t-1} + v_t,
$$

where $v_t$ is a stationary process
$v_t = \sum_{j=0}^{\infty} c_j\epsilon_{t-j}$. Let $\omega^2$ be the
long-run variance of $v_t$:

$$
\omega^2 = \sum_{-\infty}^{\infty} \gamma_j
$$

Most of the previous conclusions carry through:

$$
\begin{aligned}
\frac{1}{\sqrt T} y_t &\to \omega W(r) \\
\frac{1}{T^{3/2}}\sum_{t=1}^{T} y_t &\to \omega\int_0^1 W(r) dr
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{1}{T}\sum_{t=1}^{T} y_{t-1}v_t 
&= \frac{1}{2T}y_T^2 - \frac{1}{2T}\sum_{t=1}^{T}v_t^2 \\
&\to \frac{1}{2} [\omega^2 W^2(1) - \zeta^2] \\
&\to \omega^2\int_0^1 W dW + \frac{\omega^2-\zeta^2}{2}
\end{aligned}
$$

where $\frac{\omega^2-\zeta^2}{2}$ is a "nuisance" parameter.
