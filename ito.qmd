...
for Brownian motions, the Itô calculus, which is introduced below, is a
different class of calculus specifically designed for stochastic
...
We reformulate the intuition above to a more formal definition and
theorem.
...
Define $$\xi_T(r) = \frac{1}{\sqrt T}\sum_{i=1}^{Tr}\epsilon_i$$ where
$\epsilon_t \sim IID(0,\sigma^2)$, $0 \leq r\leq 1$. We have
$$\xi_T(r) \to \sigma W(r)$$ as $T\to\infty$.
:::

This is a reformulation of the definition in the first section. Instead
of assuming each $\epsilon_t$ has a standard deviation $\frac{1}{n}$, we
approach it with a more common formulation: assuming the standard
deviation is constant $\sigma^2$, and stabilizing it by $\sqrt{T}^{-1}$.

The Donsker's Theorem is also called the **functional central limit
theorem**. When $r=1$, the standard CLT implies

$$ \xi_T(1) = \frac{1}{\sqrt T} \sum_{i=1}^T \epsilon_i \to N(0, \sigma^2). $$
The theorem claims the convergence to normality for any $r$. As an
illustration, suppose $r=\frac{1}{2}$, then

$$ 
\xi_T\left(\frac{1}{2}\right) = \frac{1}{\sqrt T} \sum_{i=1}^{T/2}\epsilon_i = 
\frac{1}{2}\left(\frac{1}{\sqrt{T/2}} \sum_{i=1}^{T/2}\epsilon_i \right)
\to N\left(0, \frac{\sigma^2}{2}\right). 
$$

The convergence would hold for any $r$ if $T$ is large enough.
...
We abbreviate $W(t+h)$ as $W_{t+h}$. As $h\to 0$,
...
::: {#thm-itoint}
1.  $\int_{0}^{t} dW = W(t)$;
2.  $F(W_t)=\int_0^t f(W_s)dW_s$ , if $dF = f\ dW$;
3.  $F(t, W_t) = \int_0^t f(s, W_s)dW_s + \int_0^t g(s, W_s)ds$ , if
    $dF = f\ dW + g\ dt$.
:::
...
where $\epsilon_t\sim\text{WN}(0,\sigma^2)$, $t=1,2,\dots,T$. By
Donsker's theorem, it is easy to show the unit root process converges to
Brownian motion if stabilizing it by $T^{-1/2}$ :

$$
\frac{1}{\sqrt{T}}y_t = \frac{1}{\sqrt{T}}\sum_{j=1}^{t}\epsilon_j \to \sigma W(r)
$$

where $r=t/T$. Note if $y_t$ is stationary, $y_t$ will not deviate too
far from $\mathbb E(y_t)$, we would have $T^{-1/2} y_t \to 0$.

Now let's consider the behaviour of the mean
$\bar y=\frac{1}{T}\sum_{t=1}^T y_t$. Define as usual,
$\xi_T(r)=T^{-1/2}y_t$ , where $r=t/T$. We have
