...
#| echo: false
#| warning: false
#| fig-cap: "Quarterly GDP Data in Different Formats"
#| label: fig-gdpts
...
data = transform(data, date = as.yearqtr(date, format="%m/%Y"))

raw = ggplot(data, aes(x = date)) +
  geom_line(aes(y = GDP)) +
  scale_x_continuous(breaks = pretty) +
  labs(x=NULL, y="GDP (RMB Billion)")

sa = ggplot(data, aes(x = date)) +
  geom_line(aes(y = GDP)) +
  geom_line(aes(y = GDPSA), col = "red") +
  scale_x_continuous(breaks = pretty) +
  labs(x=NULL, y="GDP (RMB Billion)")

yoy = ggplot(data, aes(x = date)) +
  geom_line(aes(y = GDPYoY - 100)) +
  scale_x_continuous(breaks = pretty) +
  labs(x=NULL, y="GDP Growth (YoY %)")

qoq = subset(data, date > 2011) |>
  ggplot(aes(x = date)) +
