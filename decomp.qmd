...
#| echo: true
#| warning: false
#| fig-cap: "Quarterly GDP with 2x4-MA estimate of the trend-cycle"
#| label: fig-gdpma
...
```{r}
#| fig-cap: "Classical multiplicative decomposition of quarterly GDP"
#| label: fig-gdpdecomp
#| 
as.ts(log(data$GDP)) %>% 
  decompose() %>% 
...
The following example compares the seasonally-adjusted series using the classical decomposition with the state-of-the-art [X-13ARIMA-SEATS](https://www.census.gov/data/software/x13as.html) algorithm. Despite the former is far more rudimentary than the latter, they look quite close if we simply eye-balling the plot. By taking first-order differences, we can see the series based on classical decomposition is more volatile, suggesting the classical decomposition is less robust to unusual values.

```{r}
#| include: true
#| fig-cap: "Comparing classical decomposition and X-13"
#| fig-asp: 0.45
